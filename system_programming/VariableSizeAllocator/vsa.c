...
void* VsaAlloc(vsa_t* vsa, size_t memory_size) 
{
	...
	{
		block_size = *current_block & ~0x1;
		is_current_allocated = *current_block & 0x1;
		next_block = (void*)((char*)current_block + block_size);

		if(0 == is_current_allocated && remaining_size > block_size)
		{
			next_block_size = *next_block & ~0x1;
			is_next_allocated = *next_block & 0x1;
			if(0 == is_next_allocated)
			{
				*current_block = block_size + next_block_size;
				continue;
			}
		}
	...
	while(remaining_size > 0)
	{
		...
		if(0 == is_current_allocated && block_size >= memory_size)
		{
